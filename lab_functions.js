...
function evens(arrayName){
  for (var i = 0; i < arrayName.length; i=i+2) {
    evenNumbers.push(arrayName[i+1]);
  }
}
...
function billTot(bill) {
  for (var item of bill) {
    total = total + item.price;
  }
}

function billItemise(bill, cust) {
  for (var item of bill) {
    if (item.customer === cust) {
      customerItem.push(item.meal)
    }
  }
}
